        ...
        {expList.map((element) => (
          <div key={element.id} className="flex m-4 relative gap-8">
            ...
            <div className="border rounded-lg px-8 py-4 w-full text-center z-10 shadow-md dark:shadow-zinc-900 sm:w-96">
              ...
              <div className="text-accent-foreground mb-6 sm:mb-8 sm:text-xs">
                {element.location}
              ...
              <div className="flex flex-wrap mb-6 justify-center">
                {element.tech.map((tech, index) => {
                  return (
                    <span
                      key={index}
                      className="bg-primary border hover:bg-primary/90 border-slate-800 dark:border-slate-400 ease-in-out duration-300  backdrop-blur-xl text-primary-foreground text-sm antialiased rounded-xl px-2 py-1 m-1 cursor-pointer"
                    >
                      {tech}
                    </span>
                  );
